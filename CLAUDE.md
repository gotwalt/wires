...
- **Hosted service v1** (merge `3da4ec8`) — `wires-host` is multi-fabric: fabric registration over a `/wires/fabric/0` ALPN, per-fabric rolling retention with FIFO eviction, topic→fabric routing, write-rate ceiling, host-ticket discovery (base64 + terminal QR; `wires-host ticket` and startup-time emission). The host became a `lib + bin` crate.
...
The unit of organization rooted in one human is a **fabric** — one fabric per root key, one root key per fabric. The codebase uses this term everywhere: types (`FabricClient`, `FabricRegistry`, `FabricSupervisor`), modules (`wires-net::fabric`, `wires-host::fabric_registry`, `wires-mcp::fabrics`), the control-plane ALPN (`/wires/fabric/0`), on-disk paths (`host/fabrics/<root_hex>/`), redb table names (`fabrics.redb`), CLI commands (`wires host fabric-unregister`), and iOS UI labels. `wires-host` is multi-fabric infrastructure (one host serves many fabrics).

Earlier drafts called this concept a "tenant" (host-internal) or "household" (iOS / README marketing). Both have been retired. If you see either in code or docs that aren't historical specs/plans, fix it.
...
- **Hosted-service spec** — `docs/superpowers/specs/2026-05-14-wires-hosted-service-design.md`. Multi-fabric `wires-host`, fabric control protocol, per-fabric storage layout, HTTPS discovery surface. NOTE: the spec's §6 "InviteToken" section is **stale** — that token has been deleted in favor of the responder-driven pairing flow (see below). Treat §6 as historical.
...
| `wires-store` | redb-backed persistence: per-publisher hash-chained `topic_log` (with `bytes_stored` and `delete` for eviction), `cap_table`, `epoch_keys`, and `ingest_index` for FIFO eviction across a fabric's topics. `IngestIndex` rows carry `ingested_at_ms`; methods `evict_older_than` (TTL), `evict_oldest_until` (byte budget), `iter_all_entries` and `is_backfilled` / `set_backfilled` (one-shot backfill flag). Row decode is length-tolerant — legacy 76-byte rows fall through with `ingested_at_ms = 0` so `wires-host`'s pre-retention data keeps working. |
| `wires-net` | iroh transport: `gossip.rs` wraps `iroh-gossip`; `replay.rs` is a custom QUIC protocol on `/wires/replay/0`; `fabric.rs` is the `/wires/fabric/0` control-plane (request/response types, `FabricClient`, `FabricProtocol`); `pair.rs` is the `/wires/pair/0` responder-driven pairing protocol (`PairRequest`, `PairGrantEnvelope`, `PairFrame`, `PairProtocol`, `PairClient`, `PairHandler`); `ticket.rs` is the HostTicket module for host-ticket discovery (base64 + QR serialization); `framing.rs` is the shared length-prefixed JSON helper; `peer_hint.rs` exposes join-time endpoint discovery helpers; `endpoint.rs` exposes `bind_lan` / `bind_cloud` helpers that consolidate every `Endpoint::builder(presets::N0)…bind()` call site and wire in iroh's mDNS local discovery for on-LAN binaries. |
...
| `wires-cli` | `wires` binary — clap-based human/agent CLI. Speaks the fabric control protocol (`wires host pair / topic-register / topic-unregister / status`), drives responder-driven pairing (`wires pair-listen` on the agent, `wires pair-approve <token>` on the operator), and auto-dials gossip on `publish` / `cat`. `wires init` is identity-only by default; `wires init --new-root` is the operator path. `wires topic create` auto-mints a self-cap when run with a root key present. |
| `wires-host` | `lib + bin`. The binary is a multi-fabric blind relay; the library houses `fabric_registry` (fabrics/topic_index/nonces), `per_fabric_logs`, `retention`, `routing`, `replay_source`, and `error`. Still has no root key, no epoch keys, no caps; only persists ciphertext after a signature check, routed by topic→fabric lookup. |
...
6. **Topic→fabric routing is the host's only ACL.** Inbound envelopes whose `topic_id` is not in `topic_index.redb` are dropped (no panic, no log spam). Envelopes whose fabric is `Suspended` are dropped. Envelopes that would push a fabric past its retention budget are accepted, then the oldest entries (across all of that fabric.s topics, ordered by host-side ingest timestamp) are evicted via `IngestIndex::evict_oldest_until`. Eviction is global within a fabric, not per-topic.
...
fabrics.redb                   fabric table: root_pubkey -> FabricRecord
...
fabrics/
  <root_pubkey_hex>/
    ...
    ingest_<root_hex>.redb     per-fabric FIFO eviction index (ingest_ts + size)
```
...
- **Funnel slots.** Tailscale Funnel exposes only `443`, `8443`, `10000`
  ...
  slot from another fabric on the node or sharing one via sub-paths.
