//! Negative paths through the fabric control protocol and inbound router.
...
use wires_net::fabric::{
    FabricErrorCode, FabricHandler, FabricOp, FabricRegisterRequest, FabricResponse, signing_bytes,
};
...
fn handle_register_rejects_bad_signature() {
    ...
    match handler.handle_register(req) {
        FabricResponse::Error(e) => assert_eq!(e.code, FabricErrorCode::BadSignature),
...
fn handle_register_rejects_replayed_nonce() {
    ...
    match handler.handle_register(req) {
        FabricResponse::Error(e) => assert_eq!(e.code, FabricErrorCode::ReplayedNonce),
