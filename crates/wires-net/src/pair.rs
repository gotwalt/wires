...
/// iroh `ProtocolHandler` for `/wires/pair/0`.
///
/// One inbound connection → one bidi stream → one Grant frame in → one
/// Ack-or-Reject frame out → close. A `tokio::Mutex` serializes concurrent
/// dials so the handler observes grants strictly one at a time.
#[derive(Clone)]
