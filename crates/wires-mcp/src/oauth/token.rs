...
async fn handle_refresh(state: &ServiceState, req: &TokenRequest) -> axum::response::Response {
    ...
    match state.store.get_oauth_client(&rec.client_id) {
        Ok(Some(c)) if c.revoked => {
            return oauth_err(StatusCode::BAD_REQUEST, "invalid_grant", "client revoked");
        }
        Ok(None) => {
            return oauth_err(StatusCode::BAD_REQUEST, "invalid_grant", "unknown client");
        }
        Err(_) => return oauth_err(StatusCode::INTERNAL_SERVER_ERROR, "server_error", ""),
        Ok(Some(_)) => {}
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn refresh_rejected_when_client_revoked() {
        let tmp = TempDir::new().unwrap();
        let st = test_state(tmp.path());
        st.store
            .put_oauth_client(&OauthClientRecord {
        ...
        // Operator revokes the client after initial issuance.
        let revoked = OauthClientRecord {
            client_id: "c1".into(),
            client_name: "C".into(),
            redirect_uris: vec!["http://localhost/cb".into()],
            grant_types: vec!["authorization_code".into(), "refresh_token".into()],
        ...
        st.store.put_oauth_client(&revoked).unwrap();
