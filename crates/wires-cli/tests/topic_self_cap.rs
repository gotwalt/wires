...
async fn create_auto_mints_self_cap_when_root_present() {
    ...
    let has_write = all.values().any(|entry| {
        entry.cap.agent == agent_pk
            && entry.cap.allows("home.notes", Right::Write).is_ok()
    });
...
async fn create_twice_only_mints_one_cap_per_topic() {
    ...
    wires_cli::cmd::topic::create(td.path(), "mail.inbox").await.unwrap();
    ...
    assert!(
        all.values().any(|e| e.cap.agent == agent_pk
            && e.cap.allows("home.notes", Right::Write).is_ok()),
        "home.notes cap exists"
    );
    assert!(
        all.values().any(|e| e.cap.agent == agent_pk
            && e.cap.allows("mail.inbox", Right::Write).is_ok()),
