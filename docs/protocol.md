...
(with `newer_state` when the caller's `state_version` is older) and execs the service's fixed argv **with the caller's argv appended element by element, never through
...
verifies a token only from a member of its state (§5 step 2).
...
known principal the role admits (`member`: every member). A fetch is checked like a call: at most
64 undecided at once, membership first, and a non-member hears only `not admitted to this fabric`,
has its token left unverified, and is traced, not logged; a removed member's queue is dropped (each
message logged `denied`). A member holds at most 2 long polls open per host; a member's policy
