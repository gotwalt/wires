...
> **Archived 2026-09-22.** This was the bottom half of the README through the
> Phase 2 restart. The layer model no longer reads true as the pitch: the
> `tool.exec` frame vocabulary was never built, "multi-party as session
> fan-out" never shipped, and the product is now remote CLIs with
> IdP-authenticated callers and observable calls (see
> [docs/board](../board/README.md)). Kept for its reasoning; the "Usage"
> links below refer to the pre-2026-09-22 README.
...
> **Status.** The layer itself is built (see [Usage](../../README.md#reference) above): the
