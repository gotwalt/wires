...
use library::{CapabilityTicket, NodeId, Scope, ToolName};
...
#[derive(Subcommand)]
pub enum ToolsCmd {
    /// Add a remote tool, reached by ticket or by responder node id.
    Add(ToolsAddArgs),
...
#[derive(Args)]
#[command(group(ArgGroup::new("where").required(true).args(["ticket", "node"])))]
